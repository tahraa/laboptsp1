...
class RapportController extends Controller
{
   ...
   if(  $echantillons= $request->file('echantillons')){
    ...
    if($rapport_emp = $request->file('rapport_emp')){
        ...
        if($ordre_judiciaire = $request->file('ordre_judiciaire')){
            $mm= $ordre_judiciaire->getClientOriginalName();
            $ordre_judiciaire->move('files', $mm);
             $rapport->ordre_judiciaire = $mm;
...
public function index()
{
        ...
        $enfants = DB::table('rapports')->orderBy('num_affaire','desc')->paginate(500);
