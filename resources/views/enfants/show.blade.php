...
@endphp
    <ul>
        ...
        <li class="border border-success mt-4 p-4">
            <ul><span class="font-weight-bold text-primary">AGENT SNIM : {{ $emp->matricule }}</span>
                <li>
                        ...
                        <thead class="thead-inverse">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td><a href="{{ route('employes.show', ['employe' => $emp->id]) }}">{{ $emp->matricule }}</a></td>
                                    <td scope="row">{{ $emp->nom .' '. $emp->prenom}}</td>
                                    <td>{{ $emp->sexe}}</td>
                                    <td class="@if ($emp->statut == '1')assure @else nonAssure @endif">
                                        @if ($emp->statut == '1')
                                            assuré
                                        @endif
                                        @if ($emp->statut == '0')
                                  ...
                                  {{--   @if ($user_logged_in->profile == 'profil2')
                                    <td>
                                        <a  class="btn btn-success btn-sm" type="button" href="{{ route('employes.edit', ['employe' => $emp->id]) }}"><i class="fas fa-edit"></i></a>
                                    </td>
                                    @endif --}}
                                    @if ($user_logged_in->profile == 'profil3'||$user_logged_in->profile == 'profil2')
                                    <td>
                                        <a  class="btn btn-success btn-sm" type="button" href="{{ route('employes.edit', ['employe' => $emp->id]) }}"><i class="fas fa-edit"></i></a>
                                        <form method="POST" action="{{ route('employes.destroy', ['employe' => $emp->id]) }}">
                                            @csrf
                                            @method('DELETE')
