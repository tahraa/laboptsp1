...
@endphp
    <div class="card">
        ...
        <div class="card-header text-primary"><h3>Ajouter/Conjoint pour élement hors Snim</h3>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-md-8 offset-md-2">
                    <form action="{{ route('couplesB.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @if ($errors->any())
                        ...
                        <div class="form-group">
                            <label for="b">bénéficier</label>
                            <select id="b" required="required" id="" name="beneficier" class="form-control selectemp">
                                <option disabled selected value="vide">choisir bénéficier</option>
                                @forelse ($beneficiers as $beneficier)
                                <option {{ old('beneficier') == $beneficier->id ? 'selected' : '' }}   value="{{$beneficier->id}}">
